...
class material {
public:
	virtual bool scatter(RNG& rng, const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered) const = 0;
	virtual color emitted(RNG& rng, const ray& r_in, const hit_record& rec) const {return color(0, 0, 0);}
};
...
public:
	...
	virtual bool scatter(RNG& rng, const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered) const override{
...
public:
	...
	virtual color emitted(RNG& rng, const ray& r_in, const hit_record& rec) const override {
...
public:
	...
	virtual bool scatter(RNG& rng, const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered) const override {
		...
		scattered = ray(rec.p, reflected + fuzz * rng.random_in_unit_sphere(), r_in.wavelength);
		attenuation = albedo;
...
public:
	...
	virtual bool scatter(RNG& rng, const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered) const override {
		...
		if (rng.random_double() < fuzz) {
			scatter_direction = rec.normal + rng.random_unit_vector();
			if (scatter_direction.near_zero()) scatter_direction = rec.normal;
		}
		else
